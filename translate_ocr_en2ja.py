#!/usr/bin/env python3
"""
スキャン・画像入りPDF対応 契約書翻訳ツール【英語 → 日本語】
=====================================================================
PDFファイルをそのまま Anthropic API に送信し、Claudeのvision機能で
読み取り(テキスト+OCR)と和訳を1回のAPI呼び出しで行います。
...
使い方:
    1. 下の「★★★ 設定 ★★★」ブロックを編集
    2. 実行: python translate_ocr_en2ja.py

注意:
    ・入力はPDFのみ(Word・テキストは translate_en2ja.py を使用)
...
# ★★★ 設定(ここを編集してください)★★★
# ===========================================================================
INPUT_FILE = "samples/sample_contract_en_scan.pdf"  # 入力PDFのパス(スキャン画像PDFもOK)
...
def main() -> None:
    ...
    if input_path.suffix.lower() != ".pdf":
        sys.exit(
            "エラー: このスクリプトはPDF専用です。\n"
            "  Word・テキストファイルは translate_en2ja.py を使用してください。"
        )
